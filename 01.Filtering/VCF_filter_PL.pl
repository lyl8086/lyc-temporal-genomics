...
my ($totDP,$avgDP, $biallelic, $polymorphic, $rmIndel, $poly_thred, $small);
my $vcf_pre_header = '';
# Initialize
$num_threads = 1;
$cnt         = 0;
...
'   
    ...
    --rm    make the vcf small, only keep GT.
';
...
# use bcftools to make vcf small.
my $rmrub = "";

# I will not use rmrub here. 
if (substr($infile, -2) eq 'gz') {
    open($in_fh, "gunzip -c $infile|$rmrub") or die "$!";
    $max_per_batch = $file_stat[7]/1e6 > 100 ? 9999 : 999; # 100M
    } 
elsif (substr($infile, -2) eq '7z') {
    open($in_fh, "7zcat.sh $infile|$rmrub") or die "$!";
    $max_per_batch = $file_stat[7]/1e6 > 100 ? 9999 : 999; # 100M
    }
elsif (substr($infile, -3) eq 'bcf') {
    open($in_fh, "bcftools convert --threads 8 -Ov $infile |$rmrub") or die "$!";
    $max_per_batch = $file_stat[7]/1e6 > 100 ? 9999 : 999; # 100M
    }
else {
    open($in_fh, "$infile|$rmrub") or die "$!";
...
if ($small) {$rmrub = "| bcftools annotate -x fmt,info --threads 8 "}
if (substr($outfile, -2) eq 'gz') {
    # use bgzip.
    open($out_fh, " $rmrub | bgzip -c >$outfile") or die "$!";
    } else {
    open($out_fh, "$rmrub >$outfile") or die "$!";
}
...
if ($sort) {
    ...
    if (substr($outfile, -2) eq 'gz') {
		...
		# use bcftools
		`bcftools sort -m 200G -Oz -T tmp -W -o $outfile.vcf.gz $outfile && \
		mv $outfile.vcf.gz $outfile && \
		mv $outfile.vcf.gz.csi $outfile.csi`;
    } else {
        #`(grep '^#' $outfile; grep -v '^#' $outfile | sort -T ./ --parallel=$num_threads -k1,1V -k2,2n) >$outfile.1 && mv $outfile.1 $outfile`;
		`bcftools sort -m 200G -T tmp -o $outfile.1 $outfile && \
